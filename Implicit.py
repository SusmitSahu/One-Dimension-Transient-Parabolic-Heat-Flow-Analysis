...
print("Implicit Method for the given problem")
print("=====================================")
...
for time in range(0,int(upper_limit_time/delt)):
    ...
    for row in range(1,int(length/delx)):
        for coloumn in range(1,int(length/delx)):
           T[row,time+1] =T[row,time+1] + inverse_K[row-1,coloumn-1]*T[coloumn,time]# Coloumn here is row
    
matrix_temp = np.matrix(T)

#for time in range(0,int(upper_limit_time/delt)):
#    T[1:int(length/delx)-1,time+1] =T[1:int(length/delx)-1,time+1] + np.matmul(inverse_K,T[1:int(length/delx)-1,time])    
...
temp_implicit = pd.DataFrame(T)
file_path = "C:\\Users\\Susmit\\OneDrive\\Desktop\\Semester 6\\CME\\CME_project\\implicit_temp.xlsx"
temp_implicit.to_excel(file_path, index = False)
flux_implicit = pd.DataFrame(T)
file_path = "C:\\Users\\Susmit\\OneDrive\\Desktop\\Semester 6\\CME\\CME_project\\implicit_FLux.xlsx"
flux_implicit.to_excel(file_path, index = False)
